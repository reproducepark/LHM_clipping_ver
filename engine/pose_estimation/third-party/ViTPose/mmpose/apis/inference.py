...
def _build_batch_data(model, imgs,
                                 bboxes,
                                 dataset='TopDownCocoDataset',
                                 dataset_info=None):
    """Inference human bounding boxes.
...
def batch_inference_pose_model(model, imgs, bboxes, dataset_info=None, return_heatmap=False):
    ...
    with OutputHook(model, outputs=outputs, as_tensor=False) as h:
        ...
        batch_data = _build_batch_data(model, imgs, bboxes_xywh, 'TopDownCocoDataset', dataset_info)
        with torch.no_grad():
            result = model(
                img=batch_data['img'],
                img_metas=batch_data['img_metas'],
